...
export default function UserMetaCard() {
  ...
  const { user, loading, updateUserProfile, updateUserProfileImage } = useAuth();
  ...
  // Update form data and profile image when user data is loaded
  useEffect(() => {
    if (user) {
      setFormData({
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        ...
        // Upload to server
        const success = await updateUserProfileImage(file);
        if (success) {
          setIsImageModalOpen(false);
        }
      } catch (err) {
    ...
    return (
      <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
        <div className="flex items-center justify-center py-10">
          ...
          <span className="ml-2 text-gray-500 dark:text-gray-400">Loading profile information...</span>
        </div>
      </div>
    );
  }
